...
def shallow_feature_extract(model, ratio = 0.8, reinit_weigths = True):
    total_layers = 0
...
def prunnedResnet18(pretrained = True, freeze_ratio = 0.8, remove_classifier = True, reinit_weigths = True):
    if(pretrained): weights = ResNet18_Weights.IMAGENET1K_V1
    else: weights = ResNet18_Weights.DEFAULT
    resnet = resnet18(weights=weights)
    if(freeze_ratio != 0):
        resnet = shallow_feature_extract(resnet,ratio=freeze_ratio,reinit_weigths=reinit_weigths)
